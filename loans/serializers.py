...
class LoanSerializer(serializers.ModelSerializer):
    class Meta:
        model = Loan
        fields = ['external_id', 'amount', 'outstanding', 'status', 'contract_version', 'maximum_payment_date',
                  'customer_external_id']
        read_only_fields = ['external_id', 'outstanding', 'status', 'customer_external_id']
...
class LoanUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_status(self, value):
        ...
        if value == 2:  # active
            if current_status != 1:
                raise serializers.ValidationError("El estado debe ser 'pending' para activar el préstamo.")
            # Realizar otras validaciones si es necesario

        elif value == 3:  # rejected
            if current_status != 1:
                raise serializers.ValidationError("El estado debe ser 'pending' para rechazar el préstamo.")
            # Realizar otras validaciones si es necesario
